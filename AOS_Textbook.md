...
| Silberschatz 10e (T1) | Main text | Ch 1–6, 9–10, 20 |
| Stallings 6e Ch 3–4 | 7-state model, SVR4 diagram, PCB categories, fork steps, thread:process table | His slides copy it word for word |
| ULK 3e (Bovet & Cesati) Ch 3, 7 | Linux task states, PIDs, do\_fork, O(1) priority formulas | Lec 12–18 |
| Singhal & Shivaratri (T2) | Multiprocessor architecture now; distributed systems after the midsem | Lec 3, then Module 3 |
...
**Scope for the midsem.** The lectures so far cover architecture, design paradigms, system calls, protection and processes. Last year he also examined threads and synchronization before the midsem (15 of 75 marks came from them), and he said this year is "six classes behind". Chapters 9–10 cover that material so you are not caught out if he reaches it by 08 Oct.
...
- **Multi-user:** a different axis. Many users are logged in at once; the OS isolates them so one misbehaving user cannot hurt another, and enforces per-user **soft and hard limits** (e.g. number of processes). The hardware underneath is still multitasking.
...
| Symmetric by ISA, asymmetric by speed | Same | Different | Yes | big.LITTLE, Intel P/E cores (Sil §5.5.5) |
...
**Memory models.** Textbook (Sil §1.3.2, T2 16.3): **UMA** = every CPU sees the same memory access time; **NUMA** = each CPU has local memory that is faster than remote memory; **NORMA** = no remote memory access at all, only messages.
...
**Why distributed synchronization is hard (★):** you cannot measure message delay. Routes change, hop counts differ, queues fill and drain, so two clocks can never be set exactly equal ("1000+ algorithms"). This is the motivation for logical clocks after the midsem (T2 Ch 5).
...
**Read:** T2 16.1–16.5; Sil 1.3.1–1.3.3, 1.4.1, 1.5.5, 1.8, 5.5.2, 5.5.4–5.5.5, 3.8.2.
...
**Inode pointers (Lec 17, "make the common case fast").** He uses **10 direct** pointers (Silberschatz draws 12 — use 10 in his exam) plus single, double and triple indirect. With 4 KB blocks and 4-byte pointers, one block holds 1024 pointers:
...
**Read:** Sil 2.3, 2.8.1–2.8.5, 2.9.1, 13.1.2, 14.2, 14.4.3, 14.6, 20.2–20.3.
...
In UNIX the kernel runs **in the context of the user process** (Stallings 3.5), so a syscall is only a mode switch. If the kernel were a separate process, every mode switch would also be a context switch.
...
**Read:** Sil 1.4.2–1.4.3, 2.3.1–2.3.3, 12.2.4, 18.4–18.5; Stallings 3.5; ULK Ch 10.
...
- **External fragmentation.** Variable-size segments leave holes. The **50-percent rule** (Sil §9.2.3): with N blocks allocated, about 0.5N are lost to fragmentation, so **one-third of memory may be unusable**.
...
**Read:** Sil 9.1–9.3, 9.5, 9.6.1, 10.2.1, 10.3, 10.4.1, 10.6, 11.6; ULK Ch 2.
...
**Process image, low to high address (L11, Sil Fig 3.2):**
...
★ **Exam (2025 Q1A):** "Which variables are in .bss?" Only the **uninitialized globals/statics**. `int z = 5;` at file scope is .data; every local (and `argc`, `argv`) is on the stack. **Silberschatz §3.1.2 swaps .data and .bss — an erratum.** Use the definition above.
...
**Seven states** (Stallings Fig 3.9b) separate the two questions *waiting for an event?* and *in memory?* into four states: Ready, Blocked, Ready/Suspend, Blocked/Suspend.
...
UNIX splits Running into **User Running** and **Kernel Running**, and has both in-memory and swapped versions of Ready and Asleep (Stallings Fig 3.17).
...
**Composites:** `TASK_KILLABLE` = WAKEKILL | UNINTERRUPTIBLE (an uninterruptible sleep that a fatal signal can still end). `TASK_STOPPED` = WAKEKILL | \_\_TASK\_STOPPED. `TASK_IDLE` = UNINTERRUPTIBLE | NOLOAD. `TASK_NORMAL` = INTERRUPTIBLE | UNINTERRUPTIBLE (the set `wake_up()` targets). `EXIT_TRACE` = ZOMBIE | DEAD. The `__` prefix is a naming convention to avoid collisions — he was annoyed that lab code ignored it.
...
**Read:** Sil 3.1–3.2, 10.6; **Stallings 3.2 and 3.6** (Silberschatz has no 7-state or SVR4 diagram); ULK Ch 3 "Process State".
...
| Per-user defaults | `.bashrc`, `.bash_profile`, `.profile` |
...
**Scheduling policies:** `SCHED_FIFO`, `SCHED_RR` (real-time), `SCHED_OTHER`/`SCHED_NORMAL` (conventional; CFS today), plus `SCHED_BATCH`, `SCHED_IDLE`, `SCHED_DEADLINE` (EDF). Chapter 8 covers how they behave.

**Read:** Sil 3.1.3 (with the `task_struct` sidebar), 5.1.4, 5.7.1; **Stallings 3.3** (Table 3.5); ULK Ch 3 "Identifying a Process", "Relationships Among Processes"; ULK Ch 7 "The Scheduling Algorithm".
...
**The six UNIX steps (Stallings 3.6, his L18 slide), all done in the parent's kernel context:**
...
**Who runs first? (★)** On a uniprocessor with equal priority, **the child** — it joins the ready queue ahead of the parent, and it will usually `exec` at once, so running it first avoids needless copy-on-write faults. It is not guaranteed: they may be on different cores or queues, and often *neither* runs next because someone else is at the head of the queue.

**The quantum split (★).** The child gets **half of the parent's remaining quantum**; the parent keeps the other half. Quantum 400 ms, 300 used → 100 left → child 50, parent 50. A second fork → 25/25. Why: otherwise a process could gain CPU time just by forking. The child also stays on the parent's core (shared code, and data until COW, are already cached there).
...
**Reasons (Stallings):** normal completion, time limit exceeded, errors or failures, operator/OS intervention, parent terminated, parent request. Some systems use **cascading termination** (children die with the parent); Linux reparents them to init instead.
...
**A full process switch (Stallings):** save the processor context (PC, registers) → update the running process's PCB → move it to the right queue (ready, blocked, ready/suspend) → select another process → update its PCB → update memory-management structures → restore its context.
...
**Practice:** Silberschatz Ch 3 exercises on "how many processes are created" are exactly this style.

**Read:** Sil 3.3.1–3.3.2, 4.6.1, 10.3; **Stallings 3.5–3.6**; ULK Ch 3 "Creating Processes", "Destroying Processes".

## Chapter 8 — Scheduling on multiprocessors and in Linux (Lec 3, 10, 12, 13)

On a multicore the scheduler must decide not only *who* runs next but *where*. Linux answers with per-core run queues, periodic and on-demand migration, and affinity; within a core it runs real-time classes first and CFS for everyone else. The handout lists this as L17–L22, so expect more after the midsem, but he has already examined the pieces below.

### 8.1 Uniprocessor background you need
...
| Multilevel feedback queue | Highest non-empty queue; demote CPU hogs, promote waiters | Yes | Many parameters to tune |
...
**NUMA-aware scheduling (Sil §5.5.4):** keep a task on a core close to the memory holding its pages.
...
### 8.6 The O(1) scheduler (Linux 2.6.0–2.6.22)

- Per core, two **priority arrays**, *active* and *expired*. Each has **140 lists** (one per priority) and a **140-bit bitmap** of non-empty lists.
- Pick next: find the first set bit, take the head of that list — constant time regardless of how many tasks exist.
- A task that uses up its quantum moves to *expired* (with a recomputed quantum and dynamic priority, Chapter 6). When *active* empties, swap the two pointers.
- Interactive tasks that finish a quantum are usually put back in *active*, unless expired tasks have waited too long.
- Strength: fast and SMP-friendly. Weakness: poor response for interactive desktop work, and the interactivity heuristics were fragile. Replaced by CFS in **2.6.23**.

### 8.7 CFS: the Completely Fair Scheduler

CFS does not hand out fixed time slices. Each runnable task should get CPU in proportion to its **weight**, which comes from its nice value.

- **Weight:** nice 0 = 1024; each nice step changes weight by about 1.25× (roughly 10% CPU). Nice 5 ≈ 335.
- **Targeted latency:** an interval in which every runnable task runs at least once (Silberschatz's example: 10 ms; two equal tasks get 5 ms each). It grows when there are many tasks, with a minimum granularity per task.
- **Virtual runtime:** `vruntime` advances by actual runtime scaled by 1024/weight. At nice 0 it equals real time; a low-priority task's `vruntime` grows faster; a high-priority task's grows slower.
- **Pick next:** the task with the **smallest `vruntime`**. Runnable tasks sit in a **red-black tree** keyed by `vruntime`; the leftmost node is cached (`rb_leftmost`), so picking is O(1) and insertion O(log N).
- **I/O-bound tasks win naturally:** they sleep a lot, so their `vruntime` stays low and they preempt CPU hogs when they wake.

```latex
\text{share}_i = \frac{w_i}{\sum_j w_j}, \qquad \Delta\text{vruntime}_i = \Delta t \times \frac{1024}{w_i}
```

Example: nice 0 (1024) vs nice 5 (335) → 1024 / 1359 ≈ 75% and 25%.

**Scheduling classes:** the kernel asks each class in priority order — deadline (`SCHED_DEADLINE`, EDF), real-time (`SCHED_FIFO`/`SCHED_RR`), fair (CFS: `SCHED_NORMAL`, `SCHED_BATCH`), idle — and runs the highest-priority task of the highest non-empty class. CFS balances load across **scheduling domains** (SMT siblings, then cores sharing a cache, then NUMA nodes), preferring to move tasks between close cores.

### 8.8 Errors in the Notion notes to avoid
...
| CFS minimum quantum 5 ms → 1 ms | Unverified; CFS uses a minimum *granularity* (0.75 ms in many kernels) — don't quote a number unless he gives one |

**Interview link:** "design a job scheduler / load balancer" questions reuse exactly this vocabulary — per-worker queues, work stealing (= pull migration), affinity for cache locality, weighted fair share.

**Read:** Sil 5.1–5.3, **5.5.1–5.5.5**, **5.7.1**, 20.5; ULK Ch 7; T2 17.6 (multiprocessor scheduling, after the midsem).

## Chapter 9 — Threads (last year's pre-midsem material)
...
**Amdahl's law** (Sil §4.2) bounds the speedup on N cores when a fraction S is serial:

```latex
\text{speedup} \le \frac{1}{S + \frac{1 - S}{N}}
```

With S = 0.25 on 4 cores, speedup ≤ 1 / (0.25 + 0.1875) ≈ 2.29.
...
**Interview link:** thread pools, per-thread vs shared state, and "what happens to threads on fork" come up in backend and systems interviews; so does Amdahl's law for scaling estimates.

**Read:** Sil 4.1–4.7 (4.3 models, 4.4.1 Pthreads, 4.5.1 pools, 4.6 issues, 4.7.2 Linux threads); **Stallings 4.1** (Table 4.2).

## Chapter 10 — Synchronization (last year's pre-midsem material)
...
`count++` is three machine steps: `r = count; r = r + 1; count = r`. If a producer's `count++` and a consumer's `count--` interleave between those steps, a starting value of 5 can end as 4 or 6 (Sil §6.1). A **race condition** is any outcome that depends on the order of execution.
...
(a, b) < (c, d) means a < c, or a = c and b < d. The `choosing` array stops a process from comparing against a half-written ticket; the Notion notes omit it, but Silberschatz and Lamport include it. It gives mutual exclusion, progress and bounded waiting (FCFS by ticket).
...
**Without busy waiting** (Sil §6.6.2): each semaphore has a queue.
...
**Dining philosophers** (5 philosophers, `chopstick[5]` all 1). Each does `wait(chopstick[i]); wait(chopstick[(i+1)%5]);` eat; signal both. If all pick up the left one at once, **deadlock**. Remedies (Sil §7.1.3):
...
**Read:** Sil 6.1–6.8, 7.1–7.3 (classical problems, POSIX APIs); T2 Ch 2 for the same ideas in a distributed setting after the midsem.

## Appendix A — C, ADTs and lab conventions (Lec 6, L14)

The onlines are ADT-format C programs with full error handling; these are the conventions he grades on.

**Why ADTs:** ship only the `.h` and the `.o`, hide the implementation, and swap implementations (linear → binary search) by **relinking without recompiling the driver**. That is abstraction and encapsulation in C.

| File | Contains | His rules |
| --- | --- | --- |
| Interface `.h` | Constants, struct declarations, **`extern` prototypes**, include guards (`#ifndef X_H / #define X_H / #endif`) | Every function `extern`; **pre- and post-conditions as comments**; a dynamically allocated array appears only as an `extern` pointer |
| Implementation `.c` | `#include`s the interface; the functions | Name need not match the header's |
| Driver | `main` only, as small as possible | Allocate the array **after** reading N; handle every error return |

**Build:** `gcc -c impl.c` and `gcc -c driver.c`, then `gcc -o name impl.o driver.o` (always name the executable; `a.out` gets overwritten). Use a `Makefile`.

**Other rules:**

- `int main(int argc, char *argv[])` exactly. The return value goes to the parent (bash); check it with `echo $?`. `argv[0]` is the program name; every argument is a string; `atoi("5abc6")` = 5; use `atof` for floats.
- Input from files with `fopen`/`fscanf`, not typing and not redirection. **No `gets`/`fgets`.** Prefer `strncpy`/`strncmp`.
- Check: file won't open; N = 0 or negative; fewer values read than expected; every call's return value.
- He criticised: bubble sort without early exit, `i <= N-1` instead of `i < N`, bad indentation, unchecked preconditions.
- Traverse `a[i][j]` row by row (row-major arrays are cache-friendly); `a[j][i]` misses on every access for wide rows.
- **Learn `man`** — there is no internet in the lab. Bring only a pen and water; no phones even switched off; accounts are IP-bound to your machine.

**His "finite n" point:** at n = 10, n² + 5 beats n + 2000. OS code cares about real sizes, which is why quicksort and mergesort coexist and why random choices (pivots, cache replacement) work well in practice.

## Appendix B — After the midsem: the distributed-systems spine

Lec 17 ended with "next, AOS proper — distributed systems, hundreds of Lamport's algorithms." The primary text becomes **Singhal & Shivaratri** (T2). The order you should expect:

| T2 chapter | Topic | Key algorithms |
| --- | --- | --- |
| 4 | Architecture of distributed systems | Issues: global knowledge, naming, scalability, compatibility |
| 5 | Theoretical foundations | **Lamport logical clocks**, **vector clocks**, causal ordering (Birman–Schiper–Stephenson, Schiper–Eggli–Sandoz), global state (**Chandy–Lamport** snapshot), termination detection |
| 6 | Distributed mutual exclusion | Lamport, **Ricart–Agrawala**, Maekawa, Suzuki–Kasami, Singhal's heuristic, Raymond's tree |
| 7 | Distributed deadlock detection | Centralized, path-pushing, edge-chasing (Chandy–Misra–Haas) |
| 8 | Agreement protocols | **Byzantine generals**, Lamport–Shostak–Pease |
| 16–17 | Multiprocessor OS | Structures, synchronization, scheduling |
| 12–13 | Recovery and fault tolerance | Checkpointing, commit protocols, voting |

**Two rules to know on day one:**

- **Lamport clock:** increment before each event; on send, attach the clock; on receive, set `C = max(C, received) + 1`. Gives: a → b implies C(a) < C(b), but not the reverse.
- **Vector clock:** each process keeps a vector; increment your own entry per event; on receive, take the element-wise max, then increment your own entry. Gives: a → b **if and only if** V(a) < V(b), so concurrency is detectable.

This overlaps G526's distributed-algorithms half and your Raft project (consensus, fault tolerance): study it once, deeply, here.
...
| CFS weight | nice 0 = 1024; \~1.25× per step | 8 |

**Formulas**

```latex
\tau_{n+1} = \alpha t_n + (1-\alpha)\tau_n \qquad \text{speedup} \le \frac{1}{S + (1-S)/N} \qquad \text{EAT} = h(t_{TLB}+t_m) + (1-h)(t_{TLB}+2t_m)
```
...
| --- | --- | --- |
| NUMA | Every multicore is NUMA (own L1 < peer L1 < L3); "UMA is never used"; NORMA = loosely coupled | UMA/SMP is a real design; NUMA = local vs remote memory |
| TLB miss | Exception + OS walk + re-execute → count retries | Hardware walk; EAT formula |
| Inode | 10 direct pointers | 12 |
| i3…i9 | HT / turbo / HT + turbo / HT + multi-turbo; "never frequency" | Not covered |
| Links | Only hard and symbolic; "no such thing as a soft link" | Soft = symbolic |
| Who runs first after fork | Child (uniprocessor, equal priority), with half the remaining quantum | Unspecified |
| Offset | Per process | Shared after fork (both are true; say which case) |
| .data / .bss | .bss = uninitialized | Sil §3.1.2 swaps them (erratum) |
...
- Senior's Notion notes "ADV OS" (exported PDF in the AOS folder); errors are flagged in §8.8 and inline.
- Silberschatz, Galvin, Gagne, *Operating System Concepts*, 10th ed.; Stallings, *Operating Systems*, 6th ed., Ch 3–4; Bovet & Cesati, *Understanding the Linux Kernel*, 3rd ed., Ch 3 and 7; Singhal & Shivaratri, *Advanced Concepts in Operating Systems* — all via the coursework-textbooks connector.
